...
excerpt: Créer un README de profil <a class="gh-badge"
  ...
  />GitHub</a> exceptionnel ne consiste pas seulement à ajouter quelques badges
  — il s'agit de repousser les limites techniques. Dans cette analyse
  approfondie, j'explore la manipulation avancée de SVG, la conversion HTML en
  SVG, les animations intégrées et une automatisation complète avec GitHub
  Actions pour créer ce que je crois être l'un des READMEs GitHub les plus
  avancés techniquement. D'une Bento Grid dynamique qui se met à jour toutes les
  5 minutes à l'intégration de SVG en direct sans requêtes externes, ce projet a
  transformé mon profil en une vitrine vivante et auto-mise à jour de mon
  travail. Vous voulez savoir comment j'ai fait ? Décomposons cela. 🚀
authors:
  - trueberryless
  - ai
cover:
