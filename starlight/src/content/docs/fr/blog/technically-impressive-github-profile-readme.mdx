...
excerpt: Créer un README de profil <a class="gh-badge"
  ...
  remarquable ne se résume pas à ajouter quelques badges — il s’agit de
  repousser les limites techniques. Dans cette plongée approfondie, j'explore la
  manipulation SVG à bas niveau, la conversion HTML en SVG, les animations
  intégrées et une automatisation complète avec GitHub Actions pour concevoir ce
  que je crois être l'un des READMEs GitHub les plus avancés techniquement.
  D’une Bento Grid dynamique qui se met à jour toutes les 5 minutes à
  l’intégration de SVG en direct sans requêtes externes, ce projet a transformé
  mon profil en une vitrine vivante qui se met à jour automatiquement. Vous
  voulez savoir comment j’ai fait ? Décortiquons cela. 🚀
authors:
  - trueberryless
  - ai
cover:
